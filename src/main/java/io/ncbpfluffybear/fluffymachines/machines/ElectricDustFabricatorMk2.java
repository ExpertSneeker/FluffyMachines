...
public class ElectricDustFabricatorMk2 extends AContainer implements RecipeDisplayItem {
    ...
    public ElectricDustFabricatorMk2(ItemGroup category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
    ...
    protected MachineRecipe findNextRecipe(BlockMenu menu) {
        for (int slot : getInputSlots()) {
            for (ItemStack acceptableInput : acceptableInputs) {
                ItemStack inputItem = menu.getItemInSlot(slot);
                if (SlimefunUtils.isItemSimilar(inputItem, acceptableInput, true, false)) {
                    if (!hasFreeSlot(menu) || inputItem.getAmount() < 64) {
                        return null;
                    }

                    ItemStack dust = oreWasher.getRandomDust();
                    dust.setAmount(64); // 设置输出数量
                    MachineRecipe recipe = new MachineRecipe(4 / getSpeed(), new ItemStack[] {new ItemStack(acceptableInput.getType(), 64)},
                            new ItemStack[] {dust});

                    if (menu.fits(recipe.getOutput()[0], getOutputSlots())) {
                        inputItem.setAmount(inputItem.getAmount() - 64); // 减去64个输入物品
                        menu.replaceExistingItem(slot, inputItem);
                        return recipe;
                    }
                }
            }
